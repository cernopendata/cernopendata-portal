...
# Directly in the database would be:
# insert into pidstore_pid (created, updated, pid_type, pid_value, pid_provider, status, object_type, object_uuid)
#   select now(), now(), 'oai','oai:cernopendata.cern:' || pid_value, 'oai', status, object_type, object_uuid from pidstore_pid where pid_type='recid';
#
# update records_metadata
#    set json= json || ('{ "pids" : {"oai":{"id": "'||(json->>'recid')::text|| '"}}}')::jsonb
#    where json->>'recid' !='' ;
#
#
