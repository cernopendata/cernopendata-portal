...
def test_fix_empty_derived_slug_returns_error():
    validator = ValidSlug()
    release = DummyRelease([{"_source_filename": "some/path/.json"}])
    errors = validator.fix(release)
    assert any("Cannot derive a slug" in e for e in errors)
    assert "slug" not in release.documents[0]


def test_duplicate_pids_returns_empty_when_no_slugs():
    validator = ValidSlug()
    release = DummyRelease([{"title": "no slug"}, {"slug": ""}])
    with patch(
        "cernopendata.modules.releases.validations.pid.PersistentIdentifier"
    ) as pid_cls:
        result = validator._duplicate_pids(release)
        pid_cls.query.filter.assert_not_called()
    assert result == []


def test_duplicate_pids_returns_registered_slugs():
    validator = ValidSlug()
    release = DummyRelease([{"slug": "a"}, {"slug": "b"}])
    fake_pid = MagicMock(pid_value="a")
    with patch(
        "cernopendata.modules.releases.validations.pid.PersistentIdentifier"
    ) as pid_cls:
        pid_cls.query.filter.return_value.all.return_value = [fake_pid]
        result = validator._duplicate_pids(release)
    assert result == ["a"]


def test_duplicate_pids_returns_empty_when_query_finds_none():
    validator = ValidSlug()
    release = DummyRelease([{"slug": "a"}])
    with patch(
        "cernopendata.modules.releases.validations.pid.PersistentIdentifier"
    ) as pid_cls:
        pid_cls.query.filter.return_value.all.return_value = []
        result = validator._duplicate_pids(release)
    assert result == []
