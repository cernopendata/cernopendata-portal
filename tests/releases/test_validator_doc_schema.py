...
def test_validate_reports_when_documents_is_not_a_list(app):
    with app.app_context():
        with patch("builtins.open", mock_open(read_data=json.dumps(VALID_SCHEMA))):
            release = MagicMock()
            release.documents = {"slug": "alice-data-2015", "title": "Alice data"}
            validator = ValidDocSchema()
            errors = validator.validate(release)
    assert errors == ["The field 'documents' is not a list"]
    ...
    with app.app_context():
        with patch("builtins.open", mock_open(read_data=json.dumps(VALID_SCHEMA))):
            ...
            with patch(
                ...
                errors = ValidDocSchema().validate(release)
    assert errors == ["Could not validate the schema: Test validation error"]
