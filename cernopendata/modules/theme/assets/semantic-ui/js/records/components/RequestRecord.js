  ...
  if (availability === "requested") {
    actionButton = (
      <a href={`/stage_requests?record_id=${recordId}`} >
        <Button size="tiny" color="blue">See request status</Button>
      </a>
    );
  } else {
    actionButton = (
      <>
        <Button size="tiny" color="blue" onClick={() => setOpen(true)}>
          Request all files
        </Button>
        ...
        <Modal open={open} onClose={() => setOpen(false)} size="small">
          ...
          <Modal.Content>
            ...
            <Message warning>
              ...
              <Checkbox
                label={`I confirm that I want to request ${files} files (${size}) the data`}
                checked={confirmed}
