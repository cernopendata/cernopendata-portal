    ...
    actionButton = (
      <>
        <Button className="ml-10" size="tiny" color="blue" onClick={() => setOpen(true)}>
          Request { (file)? 'dataset':'all files' }
        </Button>

        <Modal open={open} onClose={() => setOpen(false)} size="small">
          <Modal.Header>Request to make data available</Modal.Header>
          <Modal.Content>
            <p>Please confirm you want to request {(file)?'this dataset':'all files of the record'}.</p>
            <Message warning>
              <p>This action takes time. The more data requested, the longer it will take.</p>
              <Checkbox
                label={`I confirm that I want to request ${num_files} files (${toHumanReadableSize(size)} of data)`}
                checked={confirmed}
