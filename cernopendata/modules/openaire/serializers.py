...
def dict_to_lxml(d):
    ...
    """
    _ns = {
        None: "http://namespace.openaire.eu/schema/oaire/",  # Default namespace (no prefix)
...
def openaire_etree(pid, record):
    """Openaire XML format for OAI-PMH.
...
class OpenAireSerializer(Schema):
    ...
    def get_publication_year(self, obj):
        # <datacite:dates ><datacie:date dateType="Available">2011</datacite:date></datacite:dates>
        dates = []
        if "date_published" in obj:
            dates.append(
                {"date": {"@dateType": "Available", "#text": obj["date_published"]}}
            )
        return dates
