...
def index():
    ...
    try:
        results = (
            FeaturedArticlesSearch().sort("-date_published")[:6].execute().hits.hits
        )
    except Exception:
        pass
...
def stage_requests_summary():
    ...
    if status_param:
        ...
        query = query.filter(TransferRequest.status.in_(status_list))

    if action_param:
        action_list = action_param.split(",")
        query = query.filter(TransferRequest.action.in_(action_list))

    if record_param:
        try:
            recid = int(record_param)
            query = query.filter(TransferRequest.recid == recid)
        except ValueError:
            return jsonify({"error": "Invalid record ID"}), 400

    summary = query.group_by(TransferRequest.status, TransferRequest.action).all()
