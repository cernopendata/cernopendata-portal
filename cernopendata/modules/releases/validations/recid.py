...
class ValidRecid(Validation):
    ...
    def validate(self, release):
        ...
        # If the release is not DEPLOYED, check for duplicates
        if not release.is_status(ReleaseStatus["DEPLOYED"]):
    ...
    def next_recid_start(self, release):
        ...
        max_value = (
            db.session.query(func.max(release._metadata.max_recid))
            .filter(ReleaseMetadata.experiment == release._metadata.experiment)
            .scalar()
        )
    ...
    def fix(self, release):
        ...
        for record in release.records:
            if "recid" not in record or record["recid"] in duplicates:
                counter += 1
                record["recid"] = f"{release._metadata.experiment}-{counter}"

        if release.records:
            release._metadata.max_recid = counter
        return []
