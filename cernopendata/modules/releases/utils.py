...
def user_info_with_cern_roles(remote, resp):
    ...
    print("GETTING THE USER IINFO", file=sys.stderr)
    ...
    if user:
        ...
        if remote_user:
            ...
            db.session.commit()
    print("AND RETURNING all the info", file=sys.stderr)
    return {
        "user": {
            "active": True,
            "email": email,
...
def user_payload_with_cern_roles(user):
    """Parse user payload."""
    print("IS IT IN TH EPAYLOAD", file=sys.stderr)
    ...
    client_id = current_app.config["CERN_APP_CREDENTIALS"]["consumer_key"]
    remote_user = RemoteAccount.get(user.id, client_id)
    print("RETURNING THE PAYLOAD", file=sys.sterr)
    return {
        "id": user.id,
        "email": user.email,
