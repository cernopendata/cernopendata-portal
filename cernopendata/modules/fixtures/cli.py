...
def _delete_pid(pid_field, pid_value, logger):
    """Deletes a single PID, if it still exists."""
    try:
        db.session.delete(PersistentIdentifier.get(pid_field, pid_value))
    except PIDDoesNotExistError:
        logger.warning(f"No {pid_field} PID '{pid_value}'; nothing to remove.")


def _delete_entry_pids(pid, pid_field, with_oai=False):
    """Deletes the PIDs of the version of the entry that `pid` points to.

    The concept PIDs (the ones without the '-v<version>' suffix) are shared by
    all the versions of an entry, so they are only removed when they are still
    pointing to the version that is being deleted.
    """
    logger = setup_cli_logger()
    version_value = str(pid.pid_value)
    concept_value = re.sub(r"-v\d+$", "", version_value)
    object_uuid = pid.object_uuid

    values = [version_value]
    if concept_value != version_value:
        try:
            concept_pid = PersistentIdentifier.get(pid_field, concept_value)
            if concept_pid.object_uuid == object_uuid:
                values.append(concept_value)
        except PIDDoesNotExistError:
            logger.warning(
                f"No concept {pid_field} PID '{concept_value}'; nothing to remove."
            )

    for value in values:
        _delete_pid(pid_field, value, logger)
        if with_oai:
            _delete_pid("oai", f"oai:cernopendata.cern:{value}", logger)
...
def delete_record(pid, pid_field):
    ...
    _delete_entry_pids(pid, pid_field, with_oai=True)
    return None
...
def delete_doc_or_glossary(pid, pid_field):
    ...
    _delete_entry_pids(pid, pid_field)
...
def _check_or_create_new_version(create_function, data, pid_object, pid_object_concept):
    """This function compares the current record with the provided data.

    If they are not the same, it creates a new version.
    """
