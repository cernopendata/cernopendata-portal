...
class Storage:
    ...
    def archive(self, file):
        ...
        self._logger.debug(f"Archiving a file")
        filename = file["uri"]
        # print(f"SWAPPING {self._hot_path[0]} by {self._cold_path[0]}")
        dest_file, transfer = self.find_cold_url(filename)
        if not dest_file:
            self._logger.error(f"WE CAN'T GUESS THE destination path :( of {filename}")
            return []
    ...
    def stage(self, file):
        ...
        self._logger.debug(f" Staging it")
        id = transfer.stage(filename, dest_file)
        if not id:
            self._logger.error("Error creating the transfer")
            return []
        return {
            "method_id": id,
            "method": transfer.__class__.__module__,
    ...
    def clear_hot(self, filename):
        ...
        try:
            ...
            self._logger.error(f"Error deleting the file {filename} :*( {e}")
