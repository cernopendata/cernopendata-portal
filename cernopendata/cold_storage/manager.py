...
class ColdStorageManager:
    ...
    def _is_qos(self, file, qos):
        ...
        self._logger.debug(f"  Checking if the file {file['key']} is already {qos}...")
        if qos == "cold":
    ...
    def _move_record_entry(
        ...
        if self._is_qos(file, qos):
            self._logger.debug(f" it is already {qos}")
            return []

        if Transfer().is_scheduled(file["file_id"], qos):
            self._logger.debug("It is already scheduled")
            return []
        dest_file, method = self._storage.find_cold_url(file["uri"])
        if not dest_file:
            self._logger.error(f"I can't find the cold url for {file['uri']}")
            return []
        if check_exists:
            ...
            if exists:
                ...
                return []
        if dry:
            self._logger.info("Dry run: do not issue any transfer")
            return []
    ...
    def _move_record(
        ...
        self._logger.info(f"{len(transfers)} transfers have been issued")
        return transfers
    ...
    def clear_hot(self, record_uuid, limit, dry):
        ...
        for file in self._catalog.get_files_from_record(record, limit):
            ...
            if not self._is_qos(file, "hot"):
                self._logger.debug("the file is not in hot. Ignoring it")
                continue
            self._logger.debug(f"ready to be deleted")
            if not dry:
    ...
    def check_current_transfers(self):
        ...
        self._logger.info("Checking all the ongoing transfers")
        now = datetime.utcnow()
        all_status = {}
        summary = {}
        for transfer in Transfer.get_ongoing_transfers(now):
            ...
            if transfer.status == "FAILED" or not transfer.status:
                self._logger.error(f"The transfer {id} failed :(")
                transfer.reason = error
                transfer.finished = datetime.now()
            else:
                self._logger.debug(f"Transfer {id} is in status {transfer.status}")
    ...
    def check_requests(self):
        ...
        TransferRequest.process_submitted_requests(self)

        # Now, let's look at the ones that 'started'
        TransferRequest.process_running_requests()

        # Now, the request to archive
        TransferRequest.process_archiving_requests()

        # Finally, the ones that are waiting to be archive
        TransferRequest.process_to_archive_requests(self)
