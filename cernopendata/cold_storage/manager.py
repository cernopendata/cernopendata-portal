...
class ColdStorageManager:
        ...
        if check_exists:
            ...
            if exists:
                if register:
                    ...
                    print("The size or the checksum is different: {file['size']} vs {exists['size']}")
                    return []
            print(
                "The file already exists in the destination storage... Should it be registered (hint: `--register`)?"
            )
            return []
        if dry:
    ...
    def check_current_transfers(self):
        ...
        for transfer in Transfer.get_ongoing_transfers(now):
            ...
            status, error = self._storage._transfers[transfer.method].transfer_status(
                id
            )
