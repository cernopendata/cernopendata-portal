...
class TransferRequest(db.Model):
    ...
    @classmethod
    def process_archiving_transfers(cls):
        ...
        for transfer in transfers:
            ...
            for index in record.file_indices:
                for f in index["files"]:
                    if not "tags" in f or not "uri_cold" in f["tags"]:
                        print(f"The file {f} is not in tape yet...")
                        finished = False
    ...
    @classmethod
    def process_to_archive_transfers(cls, manager):
        ...
        if active_transfers_count < cls.ARCHIVE_THRESHOLD:
            ...
            submitted = 0
            for transfer in transfers:
                print(f"Submitting a transfer for {transfer.record_id}")
                info = manager.doOperation(
                    "archive",
                    transfer.record_id,
                    limit=-1,
                ...
                if submitted + active_transfers_count > cls.ARCHIVE_THRESHOLD:
                    print(
                        "Reached the threshold of transfers. Let's wait before continuing"
                    )
                    break
